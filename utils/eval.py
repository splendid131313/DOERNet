...
def measure_mea(mask_name, gt_root, pred_root):
...
def eval(gt_path,
         pred_path,
         dataset_name):
    global gt_root, pred_root
    gt_root = os.path.join(gt_path, 'GT')
    pred_root = os.path.join(pred_path)
    # for dataset in ['COD10K', 'CAMO', 'CHAMELEON']:
    gt_name_list = sorted(os.listdir(gt_root))

    measure_func = partial(measure_mea, gt_root=gt_root, pred_root=pred_root)

    res = process_map(measure_func, gt_name_list, max_workers=8, chunksize=4)
